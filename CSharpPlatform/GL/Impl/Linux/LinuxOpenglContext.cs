...
namespace CSharpPlatform.GL.Impl.Linux
{
	unsafe public class LinuxOpenglContext : IOpenglContext
	{
		...
		[DllImport("libX11")] public static extern IntPtr XCreateSimpleWindow(IntPtr display, IntPtr parent, int x, int y, int width, int height, int border_width, int border, int background);
		[DllImport("libX11")] public static extern IntPtr XRootWindow(IntPtr display, int screen_number);
		[DllImport("libX11")] public static extern IntPtr XCreateColormap(IntPtr display, IntPtr window, IntPtr visual, int alloc);
		[DllImport("libX11")] public static extern void XMapWindow(IntPtr display, IntPtr window);
		[DllImport("libX11")] public extern static int XInitThreads();
		...
		private LinuxOpenglContext(IntPtr WindowHandle)
		{
			...
			if (WindowHandle == IntPtr.Zero)
			{
				...
				uint mask = (uint)SetWindowValuemask.ColorMap | (uint)SetWindowValuemask.EventMask | (uint)SetWindowValuemask.BackPixel | (uint)SetWindowValuemask.BorderPixel;
